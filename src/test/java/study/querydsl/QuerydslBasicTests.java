...
//        List<Member> content = results.getResults();
     ...
     * 1. 회원 나이 내림차순
     * 2. 회원 이름 올림차순
     * 단 2에서 회원 이름이 없으면 마지막에 출력(nulls lats)
     */
    @Test
    void sort() {
    ...
    void aggregation() {
        ...
        // Tuple보단 DTO를 정의해서 받아오는 것을 더 많이 사용
...
//                .having(team.name.contains("A"))
    ...
    void join_on_no_relation() {
        ...
        List<Tuple> result = query
                .select(member, team)
                .from(member)
                .leftJoin(team).on(member.username.eq(team.name)) // 연관관계 x, 외부 조인
                .fetch();
    ...
    void fetchJoinNo() {
        ...
        // team이 로딩 된 엔티티인지 아니면 초기화가 안된 엔티티인지 알려줌
    ...
    @Test
    void findDtoByJPQL() {
        String sql = "select new study.querydsl.dto.MemberDto(m.username, m.age) from Member m";
        List<MemberDto> result = em
                .createQuery(sql, MemberDto.class)
                .getResultList();
